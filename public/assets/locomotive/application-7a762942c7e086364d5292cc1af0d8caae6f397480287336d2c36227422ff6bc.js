...
(function() {
  ...
  Locomotive.Views.ContentEntries.IndexView = (function(superClass) {
    ...
    IndexView.prototype.initialize = function() {
      this.list_view = new Locomotive.Views.Shared.ListView({
        el: this.$('.big-list')
      });
      return this.bulk_delete_view = new Locomotive.Views.Shared.BulkDeleteView();
    };

    IndexView.prototype.render = function() {
      this.list_view.render();
      return this.bulk_delete_view.render();
    };

    IndexView.prototype.remove = function() {
      this.list_view.remove();
      this.bulk_delete_view.remove();
...
(function() {
  ...
  Locomotive.Views.ContentEntries.NewView = (function(superClass) {
    ...
    NewView.prototype.render = function() {
      ...
      return $('.tab-pane .has-error').each(function() {
        var $tab, $tab_pane_id;
        $tab_pane_id = $(this).parents('.tab-pane').attr('id');
        $tab = $("[href='#" + $tab_pane_id + "']");
        return $tab.addClass('has-errors');
      });
    };
...
(function() {
  ...
  Locomotive.Views.CurrentSite.EditView = (function(superClass) {
    ...
    EditView.prototype.initialize = function() {
      ...
      return this.allow_url_redirections_expert_mode();
    };
    ...
    EditView.prototype.allow_url_redirections_expert_mode = function() {
      return this.$('#site_url_redirections_expert_mode').on('switchChange.bootstrapSwitch', function(event, state) {
        var $inputs;
...
(function() {
  ...
  Locomotive.Views.Inputs.RteView = (function(superClass) {
    ...
    RteView.prototype.register_editor_events = function() {
      ...
      count = this.editor.currentView.doc.body.innerText.length;
      this.$counter = $("<div class='wysihtml5-counter'>" + count + "</div>").insertAfter(this.$('> .form-wrapper'));
      $(this.editor.currentView.doc.body).on('keyup', (function(_this) {
        return function(event) {
          return _this.$counter.html(event.currentTarget.innerText.length);
        };
      })(this));
...
(function() {
  ...
  (base = Locomotive.Views).PageContent || (base.PageContent = {});

  Locomotive.Views.PageContent.EditView = (function(superClass) {
    extend(EditView, superClass);
    ...
    EditView.prototype.el = '.content-main';
    ...
    EditView.prototype.initialize = function() {
      return _.bindAll(this, 'shrink_preview', 'close_sidebar');
    };
    ...
    EditView.prototype.shrink_preview = function(event) {
      return $(this.el).removeClass('actionbar-closed');
    };

    EditView.prototype.close_sidebar = function(event) {
      return PubSub.publish('sidebar.close');
    };
...
(function() {
  ...
  Locomotive.Views.Shared.BulkDeleteView = (function(superClass) {
    extend(BulkDeleteView, superClass);

    function BulkDeleteView() {
      return BulkDeleteView.__super__.constructor.apply(this, arguments);
    }

    BulkDeleteView.prototype.el = '.main';

    BulkDeleteView.prototype.events = {
      'change .list-main-checkbox': 'tickOrUntickAll',
      'change .item input[type=checkbox]': 'tickOrUntick'
    };

    BulkDeleteView.prototype.tickOrUntickAll = function(event) {
      this.$('.item input[type=checkbox]').prop('checked', event.target.checked);
      return this.refreshBulkDestroyFormState();
    };

    BulkDeleteView.prototype.tickOrUntick = function(event) {
      this.$('.list-main-checkbox').prop('checked', false);
      return this.refreshBulkDestroyFormState();
    };

    BulkDeleteView.prototype.refreshBulkDestroyFormState = function() {
      var ids;
      ids = [];
      this.$('.item input[type=checkbox]:checked').each(function() {
        return ids.push($(this).val());
      });
      this.$('.bulk-destroy-action input[name=ids]').val(ids.join(','));
      if (this.$('.item input[type=checkbox]:checked').size() > 0) {
        return this.$('.bulk-destroy-action').removeClass('hidden');
      } else {
        return this.$('.bulk-destroy-action').addClass('hidden');
      }
    };

    return BulkDeleteView;
