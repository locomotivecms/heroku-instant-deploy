...
(function() {
  ...
  Locomotive.Views.Shared.FormView = (function(superClass) {
    ...
    FormView.prototype.remove = function() {
      ...
      this.$('.input.select select').select2('destroy');
...
(function() {
  ...
  Locomotive.Views.EditableElements.PageView = (function(superClass) {
    ...
    PageView.prototype.refresh_image = function(msg, data) {
      return this.refresh_elements('image', data.view, (function(_this) {
        return function($elements, element_id) {
          var current_image_url, image_url;
          current_image_url = data.view.$('input[name*="[content]"]').val();
          image_url = data.url || current_image_url;
          if ($elements.size() > 0) {
            return _this.replace_images($elements, image_url);
          } else {
            return _this.replace_images_identified_by_url(current_image_url, image_url, element_id);
          }
        };
      })(this));
    };

    PageView.prototype.refresh_elements = function(type, view, callback) {
      var $form_view, element_id;
      $form_view = $(view.el).parent();
      element_id = $form_view.find('input[name*="[id]"]').val();
      return callback($(this.el).find("*[data-element-id=" + element_id + "]"), element_id);
    };
    ...
    PageView.prototype.replace_images_identified_by_url = function(current_image_url, new_image_url, element_id) {
      var $el;
      $el = $(this.el).find("*[style*='" + current_image_url + "']").attr('data-element-id', element_id);
      $el.css("background-image", "url('" + new_image_url + "')");
      $el = $(this.el).find("img[src*='" + current_image_url + "']").attr('data-element-id', element_id);
      return $el.attr('src', new_image_url);
    };
  ...
  Locomotive.Views.EditableElements.TextHighLighterView = (function(superClass) {
    ...
    TextHighLighterView.prototype.hide = function(event) {
      return this.hiding_timeout = setTimeout(((function(_this) {
        return function() {
          return _this.$('.locomotive-highlighter-text').hide();
        };
      })(this)), 600);
    };
...
(function() {
  ...
  Locomotive.Views.Inputs.ArrayView = (function(superClass) {
    ...
    ArrayView.prototype.make_remote_selectable = function() {
      Select2.helpers.build(this.$new_input);
...
(function() {
  ...
  Locomotive.Views.Inputs.RteView = (function(superClass) {
    ...
    RteView.prototype.initialize = function() {
      _.bindAll(this, 'register_keydown_event', 'on_content_change', 'resize');
    ...
    RteView.prototype.render_views = function() {
      ...
      return setTimeout(function() {
        return that.views = [that.build_and_render_view(Locomotive.Views.Inputs.Rte.LinkView), that.build_and_render_view(Locomotive.Views.Inputs.Rte.FileView), that.build_and_render_view(Locomotive.Views.Inputs.Rte.TableView), that.build_and_render_view(Locomotive.Views.Inputs.Rte.EditTableView)];
      }, 200);
    };
